  ...
  return (
    <>
      ...
      <div className={`cart-drawer ${cartOpen ? "open" : ""}`}>
        ...
        <div className="cart-body" zIndex={9999}>
          {cartItems.length === 0 ? (
            <p className="text-center py-5">No items in cart</p>
          ) : (
            cartItems.map((item) => (
              /* Use combined id and size for a truly unique key */
              <div className="cart-item" key={`${item.id}-${item.price}`}>
                <img src={item.image} alt={item.name} />

                <div className="cart-item-info">
                  <p className="cart-item-title">{item.name}</p>
                  <small><strong>{item.productName.length > 20 ? item.productName.slice(0, 20) + "..." : item.productName}</strong></small>
                  <small>price: {item.price}</small>

                  <div className="cart-item-bottom d-inline-block">
                    <div className="qty-control mb-2">
                      <button
                        className="qty-btn decrease m-0 p-0"
                      ...
                      <button
                        className="qty-btn increase m-0 p-0"
